 ...
 */
async function updatePriceTXB(client: SuiClient, txb: Transaction) {
...
/**
 * Updates the Oracle PTB (Price Time Blockchain) by performing the following steps:
 * 1. Creates a new test transaction.
 * 2. Calls the `updatePriceTXB` function to update the test transaction with price information.
 * 3. Inspects the test transaction using the `devInspectTransactionBlock` method of the `client` object.
 * 4. If there is an error in the inspection result, throws an error.
 * 5. If there is an error during the inspection, sets the `needSkip` flag to true.
 * 6. If the `needSkip` flag is true, returns early from the function.
 * 7. Retrieves the Pyth stale price feed IDs from the `PriceFeedConfig` object.
 * 8. Calls the `getPythStalePriceFeedId` function to get the stale price feed IDs.
 * 9. If there are stale price feed IDs, calls the `updatePythPriceFeeds` function to update the price feeds in the transaction.
 * 10. Logs a message indicating the request to update the Pyth price feeds.
 * 11. Calls the `updateSinglePrice` function for each price feed in the `PriceFeedConfig` object.
 *
 * @param client - The SuiClient object used to interact with the blockchain.
 * @param address - The address of the sender.
 * @param txb - The Transaction object to update with price information.
 * @returns void
 */
export async function updateOraclePTB(client: SuiClient, address: string, txb: Transaction) {
    const testTX = new Transaction()
    await updatePriceTXB(client, testTX)
    let needSkip = false

    try {
        const result = await client.devInspectTransactionBlock({
            transactionBlock: testTX,
            sender: address,
        })
        if (result.error) {
