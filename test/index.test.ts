...
describe('test Migration', async () => {
    ...
    it('should migrate supply from USDC to Sui', async () => {
        ...
        txb.setSender(account.address);
        const amount = 1e6;
        await migrateSupplyPTB(txb, nUSDC, Sui, amount, account.address, apiKey, account.client);
    ...
    it('should migrate supply from Sui to USDC', async () => {
        ...
        txb.setSender(account.address);
        const amount = 1e6;
        await migrateSupplyPTB(txb, Sui, nUSDC, amount, account.address, apiKey, account.client);
        const dryRunResult = await dryRunTXB(txb, account.client);
        expect(dryRunResult.effects.status.status).toEqual("success");
    ...
    it('should migrate borrow from Sui to USDC', async () => {
        ...
        txb.setSender(account.address);
        const amount = 1e6;
        await migrateBorrowPTB(txb, Sui, nUSDC, amount, account.address, apiKey, account.client);
        const dryRunResult = await dryRunTXB(txb, account.client);
        expect(dryRunResult.effects.status.status).toEqual("success");
    ...
    it('should migrate repay from USDC to Sui', async () => {
        ...
        txb.setSender(account.address);
        const amount = 1e6;
        await migrateBorrowPTB(txb, nUSDC, Sui, amount, account.address, apiKey, account.client);
        const dryRunResult = await dryRunTXB(txb, account.client);
        expect(dryRunResult.effects.status.status).toEqual("success");
    }); 
