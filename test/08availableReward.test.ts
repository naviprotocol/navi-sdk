...
describe("chain query test", () => {
  ...
  it("should success get v2 supply  available rewards ", async () => {
    const txRes = await v2.getAvailableRewards(account.client, account.address, 1);
  ...
  it("should success get v2/v3 supply  available rewards ", async () => {
    const txRes = await getAvailableRewards(account.client, account.address, [1, ]);
    console.log(JSON.stringify(txRes, null, 2));
  }, 5000000);

  it("should success get v2/v3 borrow  available rewards ", async () => {
    const txRes = await getAvailableRewards(account.client, account.address, [3, ]);
    console.log(JSON.stringify(txRes, null, 2));
  }, 5000000);

  it("should success get v2/v3 all available rewards ", async () => {
    const txRes = await getAvailableRewards(account.client, account.address, [1, 3]);
    console.log(JSON.stringify(txRes, null, 2));
  }, 5000000);
});
