...
describe("buildSwapPTBFromQuote test", () => {
  ...
  it("should swap PTB with old fee option successfully", async () => {
    const testCaseName = expect.getState().currentTestName || "test_case";
    const txb = createTransaction(account);
    ...
    const txResult = await handleTransactionResult(txb, account, testCaseName, true, true);
    expect(txResult).toEqual("success");
  }, 500000);

  it("should swap PTB with new fee option successfully", async () => {
    const testCaseName = expect.getState().currentTestName || "test_case";
    const txb = createTransaction(account);
